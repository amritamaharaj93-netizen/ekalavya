...
// Fetch Study Centers (Excluding the online platform card as it's not a physical location)
$centers = $pdo->query("SELECT * FROM study_centers WHERE name NOT LIKE '%ONLINE LIVE%' ORDER BY is_head_office DESC, created_at ASC")->fetchAll();
?>
...
<section class="contact-hub py-6 position-relative bg-white" id="connect">
    <div class="container container-1440 position-relative z-1">
        <div class="row g-4 align-items-center">
            ...
            <?php 
            // We want to place the Enquiry Form in the middle (column 2)
            // If we have at least one center, place it in column 1
            // If we have more, place them after the form or in next rows
            
            $center_count = count($centers);
            $first_center = $center_count > 0 ? $centers[0] : null;
            $other_centers = array_slice($centers, 1);
            ?>
            ...
            <div class="col-lg-4">
                ...
                <div class="location-card p-4 p-md-5 rounded-5 text-white h-100 transition-all hover-translate-y position-relative overflow-hidden d-flex flex-column" style="background-color: #0d1b31; border: 1px solid rgba(255,255,255,0.05);">
                    <?php if($first_center['is_head_office']): ?>
                    ...
                    <div class="d-flex align-items-center gap-3 mb-4">
                        <div class="icon-box bg-white bg-opacity-10 text-warning rounded-4 d-flex align-items-center justify-content-center" style="width: 50px; height: 50px; font-size: 1.2rem;">
                            <i class="fas <?php echo $first_center['is_head_office'] ? 'fa-building' : 'fa-map-marker-alt'; ?>"></i>
                        </div>
                        <h3 class="fw-black mb-0 text-uppercase text-warning fs-4"><?php echo htmlspecialchars($first_center['name']); ?></h3>
                    </div>
                    
                    <p class="text-light opacity-75 mb-5 fs-6"><?php echo htmlspecialchars($first_center['address']); ?></p>
                    
                    <div class="contact-info-list d-flex flex-column gap-4 mb-5">
                        <?php if($first_center['phone']): ?>
                        <div class="d-flex align-items-center gap-3">
                            <i class="fas fa-phone-alt text-warning"></i>
                            <span class="fw-bold"><?php echo htmlspecialchars($first_center['phone']); ?></span>
                        </div>
                        <?php endif; ?>
                        <?php if($first_center['email']): ?>
                        <div class="d-flex align-items-center gap-3">
                            <i class="fas fa-envelope text-warning"></i>
                            <span class="fw-bold"><?php echo htmlspecialchars($first_center['email']); ?></span>
                        </div>
                        <?php endif; ?>
                        <div class="d-flex align-items-center gap-3">
                            <i class="fas fa-clock text-warning"></i>
                            <span class="fw-bold">Open: <?php echo htmlspecialchars($first_center['open_time']); ?></span>
                        </div>
                    </div>
                    
                    <div class="mt-auto">
                        <a href="<?php echo htmlspecialchars($first_center['map_link'] ?: 'https://maps.google.com'); ?>" target="_blank" class="btn btn-outline-light w-100 py-3 rounded-pill fw-bold border-opacity-25">
                            View On Map <i class="fas fa-paper-plane ms-2"></i>
                        </a>
                    </div>
                </div>
                <?php else: ?>
                    <div class="text-center p-5 bg-light rounded-5 opacity-50">
                        <i class="fas fa-map-marked-alt display-4 mb-3"></i>
                        <p>Center details coming soon</p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="col-lg-4">
                <div class="contact-form-premium p-4 p-md-5 rounded-5 bg-white position-relative h-100 shadow-lg text-center">
                    ...
                    <form id="contactEnquiryForm" action="process-contact.php" method="POST" class="row g-4 text-start">
                        ...
                        <div class="col-12">
                            <input type="tel" name="phone" class="form-control form-control-lg border-0 bg-light rounded-4 px-4 py-3" placeholder="Phone Number" maxlength="10" pattern="[0-9]{10}" oninput="this.value = this.value.replace(/[^0-9]/g, '');" required>
                        </div>
                        <div class="col-12">
            ...
            <!-- Right: Other Centers -->
            <div class="col-lg-4">
                <?php if ($other_centers): ?>
                    <div class="vstack gap-4 h-100">
                    <?php foreach($other_centers as $center): ?>
                        <div class="location-card p-4 p-md-5 rounded-5 text-white flex-grow-1 transition-all hover-translate-y position-relative overflow-hidden d-flex flex-column" style="background-color: #0d1b31; border: 1px solid rgba(255,255,255,0.05);">
                            ...
                            <div class="d-flex align-items-center gap-3 mb-4">
                                <div class="icon-box bg-white bg-opacity-10 text-warning rounded-4 d-flex align-items-center justify-content-center" style="width: 50px; height: 50px; font-size: 1.2rem;">
                                    <i class="fas <?php echo $center['is_head_office'] ? 'fa-building' : 'fa-map-marker-alt'; ?>"></i>
                                </div>
                                <h3 class="fw-black mb-0 text-uppercase text-warning fs-4"><?php echo htmlspecialchars($center['name']); ?></h3>
                            </div>
                            
                            <p class="text-light opacity-75 mb-5 fs-6"><?php echo htmlspecialchars($center['address']); ?></p>
                            
                            <div class="contact-info-list d-flex flex-column gap-4 mb-5">
                                <?php if($center['phone']): ?>
                                <div class="d-flex align-items-center gap-3">
                                    <i class="fas fa-phone-alt text-warning"></i>
                                    <span class="fw-bold"><?php echo htmlspecialchars($center['phone']); ?></span>
                                </div>
                                <?php endif; ?>
                                <?php if($center['email']): ?>
                                <div class="d-flex align-items-center gap-3">
                                    <i class="fas fa-envelope text-warning"></i>
                                    <span class="fw-bold"><?php echo htmlspecialchars($center['email']); ?></span>
                                </div>
                                <?php endif; ?>
                                <div class="d-flex align-items-center gap-3">
                                    <i class="fas fa-clock text-warning"></i>
                                    <span class="fw-bold">Open: <?php echo htmlspecialchars($center['open_time']); ?></span>
                                </div>
                            </div>
                            
                            <div class="mt-auto">
                                <a href="<?php echo htmlspecialchars($center['map_link'] ?: 'https://maps.google.com'); ?>" target="_blank" class="btn btn-outline-light w-100 py-3 rounded-pill fw-bold border-opacity-25">
                                    View On Map <i class="fas fa-paper-plane ms-2"></i>
                                </a>
                            </div>
                        </div>
                ...
                <?php else: ?>
                    <div class="text-center p-5 bg-light rounded-5 opacity-50 h-100 d-flex flex-column justify-content-center">
                        <i class="fas fa-building display-4 mb-3"></i>
