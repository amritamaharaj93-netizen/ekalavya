...
if (isset($_POST['add_student'])) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO students (student_id, name, email, phone, program, password, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
                ...
                <tbody>
                    <?php if ($students): ?>
                        <?php foreach ($students as $i => $s): 
                            $admit_exists = (!empty($s['admit_card']) && file_exists('../uploads/admit_cards/' . $s['admit_card']));
                        ?>
                        <?php
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <button class="btn-manage action-btn text-primary"
                                            ...
                                            data-admit-exists="<?php echo $admit_exists ? '1' : '0'; ?>"
                        ...
                        <form action="manage-students.php" method="POST">
                            ...
                            <div class="row g-3 mb-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="phone" id="edit_phone" class="form-control rounded-3" maxlength="10" pattern="[0-9]{10}" oninput="this.value = this.value.replace(/[^0-9]/g, '');" required>
                                </div>
                                <div class="col-md-6">
            ...
            <div class="modal-body p-4">
                <form action="manage-students.php" method="POST">
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-12">
                            <label class="small fw-bold text-muted mb-1">Phone Number</label>
                            <input type="text" name="phone" class="form-control rounded-3" maxlength="10" pattern="[0-9]{10}" oninput="this.value = this.value.replace(/[^0-9]/g, '');" required placeholder="10-digit number">
                        </div>
                        <div class="col-12">
                            <label class="small fw-bold text-muted mb-1">Academic Program</label>
...
manageModal.addEventListener('show.bs.modal', event => {
    ...
    const admitExists = btn.getAttribute('data-admit-exists');
    
    if (admit && admitExists === '1') {
        box.innerHTML = `
            <div class="d-flex align-items-center justify-content-between p-3 rounded-3" style="background:rgba(16,185,129,.08); border:1.5px solid rgba(16,185,129,.2);">
                <div class="d-flex align-items-center">
                    ...
                    <span class="small fw-semibold text-success">Admit card already uploaded</span>
                </div>
